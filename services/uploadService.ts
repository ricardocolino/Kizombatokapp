...
/**
 * Faz a mixagem de áudio e vídeo no Cloudflare Mix Worker e faz upload do resultado.
 */
export async function mixAndUploadToR2(
  videoFile: File | Blob,
  audioUrl: string,
  folder: string,
  fileName?: string,
  ...
  const endpoint = "https://proud-feather-1347.anastacia6000.workers.dev/";
  ...
  console.log(`>>> [MIX WORKER] Enviando para: ${endpoint} com áudio: ${audioUrl}`);
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          const data = JSON.parse(xhr.responseText);
          if (!data.url) {
            reject(new Error("O servidor de mixagem não devolveu a URL do ficheiro final."));
          } else {
            resolve(data.url);
          }
        } catch {
          // Se não for JSON válido, tenta ver se tem URL no texto ou se devolve erro
          if (xhr.responseText.includes("url")) {
            try {
              const matched = xhr.responseText.match(/"url"\s*:\s*"([^"]+)"/);
          ...
          reject(new Error("Erro ao processar resposta JSON do servidor de mixagem."));
        }
      } else {
        reject(new Error(`Erro ${xhr.status} no servidor de mixagem: ${xhr.responseText.slice(0, 100)}`));
      }
    };

    xhr.onerror = () => {
      reject(new Error("Erro de conexão com o servidor de mixagem"));
    };
