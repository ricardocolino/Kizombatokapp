    ...
    <div className="flex flex-col h-full relative overflow-hidden bg-transparent">
        ...
        <div className="space-y-1.5 pt-2">
          {notices.map(notice => (
            <div key={notice.id} className="flex items-center gap-1.5 animate-in slide-in-from-left duration-500 fade-in">
              <div className={`w-1.5 h-1.5 rounded-full ${notice.type === 'join' ? 'bg-emerald-500' : 'bg-purple-600'} animate-pulse`} />
      ...
      <div className="p-3 flex items-center gap-2">
        <form onSubmit={handleSendMessage} className="flex-1 flex items-center gap-2 min-w-0">
          <div className={`flex-1 min-w-0 relative group flex items-center backdrop-blur-md border rounded-full px-4 py-2 transition-all shadow-lg ${isSilenced ? 'bg-purple-600/10 border-purple-600/20' : 'bg-white/10 border-white/20 focus-within:bg-white/20 focus-within:border-white/30'}`}>
            <input
              type="text"
              value={newMessage}
