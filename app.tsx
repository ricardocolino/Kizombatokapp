    ...
    try {
      ...
      if (isVideo && !isFromGallery) {
        ...
        try {
          ...
          // BUG FIX: trim só é aplicado quando trimStart > 0 ou trimEnd é menor que a duração gravada
          const hasTrim = trimStart > 0 || (recordingSeconds > 0 && trimEnd < recordingSeconds);
          if (hasTrim) {
          ...
          // BUG FIX: Mix de áudio reutilizado (dublagem)
          // Se existir áudio reutilizado, fazemos download e escrevemos no FFmpeg
          let hasReusedAudio = false;
          if (reusedAudioUrl) {
            try {
              const audioResponse = await fetch(reusedAudioUrl);
              const audioBlob = await audioResponse.blob();
              const audioData = await fetchFile(audioBlob);
              await ffmpeg.writeFile('/reused_audio', audioData);
              videoArgs.push('-i', '/reused_audio');
              hasReusedAudio = true;
              console.log('[FFmpeg] Áudio reutilizado carregado:', reusedAudioUrl);
            } catch (audioErr) {
              console.error('[FFmpeg] Erro ao carregar áudio reutilizado:', audioErr);
              // Continua sem o áudio reutilizado como fallback
            }
          }
          ...
          // Se tiver áudio reutilizado, misturamos: silencia o áudio original e usa o reutilizado
          if (hasReusedAudio) {
            videoArgs.push(
              '-filter_complex', '[0:a][1:a]amix=inputs=2:duration=first:dropout_transition=0:weights=0 1[aout]',
              '-map', '0:v',
              '-map', '[aout]',
            );
          }
          ...
          console.log('[FFmpeg Background] Executando:', videoArgs.join(' '));
          ...
          if (hasReusedAudio) { try { await ffmpeg.deleteFile('/reused_audio'); } catch { /* ignore */ } }
      ...
      if (uploadType === 'story') {
        ...
        const { error: insertError } = await supabase.from('posts').insert({
          ...
          is_ready: true,
          views: 0,
          // Guardar referência ao post de áudio reutilizado (dublagem)
          ...(reusedAudioPostId ? { reused_audio_post_id: reusedAudioPostId } : {}),
